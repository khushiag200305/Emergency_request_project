...
def assign_vehicle(req_id):
    """
    Fixed vehicle assignment:
      - Calls stored procedure AssignVehicle correctly
      - Passes operator_id as session user so Dispatch_Record.operator_id is populated
      - Then auto-assigns staff via AssignStaffToRequest procedure
    """
    ...
    try:
        ...
        # Step 1: Get emergency type for staff assignment later
        cur.execute("SELECT emergency_type FROM Emergency_Request WHERE request_id=%s", (req_id,))
        row = cur.fetchone()
        etype = row[0] if row else "Medical"

        # Step 2: Call AssignVehicle stored procedure
        cur.execute("SET @out_msg = ''")
        cur.execute(f"CALL AssignVehicle({req_id}, @out_msg)")
        conn.commit()
        cur.execute("SELECT @out_msg AS msg")
        row = cur.fetchone()
        msg = row[0] if row else "Unknown result"

        # Step 3: If assignment succeeded, update operator_id in Dispatch_Record
        #         (The stored procedure does not set operator_id — we patch it here)
        if msg and msg.startswith("SUCCESS"):
            ...
            # Step 4: Auto-assign staff
            cur.execute("SET @staff_cnt = 0")
            cur.execute(f"CALL AssignStaffToRequest({req_id}, '{etype}', @staff_cnt)")
            conn.commit()
            cur.execute("SELECT @staff_cnt AS cnt")
            sc = cur.fetchone()
            staff_count = sc[0] if sc else 0
...
def admin_dashboard():
    ...
    # Query 19: Area response stats table (uses Area_Response_Stats)
    area_stats = query(
...
@role_required("Admin")
def admin_vehicles():
    # Query 21: Vehicles with maintenance status and dispatch count (subquery)
    vehicles = query(
        """SELECT ev.vehicle_id, ev.vehicle_type, ev.license_plate,
                  ev.availability, ev.driver_name, ev.driver_phone,
                  sc.center_name, sc.area, sc.service_type,
                  vm.status AS maint_status, vm.next_service_date, vm.remarks AS maint_remarks,
                  (SELECT COUNT(*) FROM Dispatch_Record dr
                   WHERE dr.vehicle_id = ev.vehicle_id) AS total_dispatches
           FROM Emergency_Vehicle ev
           JOIN Service_Center sc ON ev.center_id = sc.center_id
           LEFT JOIN Vehicle_Maintenance vm ON ev.vehicle_id = vm.vehicle_id
           ORDER BY ev.vehicle_type, ev.vehicle_id"""
    )

    # Query 22: Vehicles needing service (uses Maintenance_Due view)
    maintenance_due = query("SELECT * FROM Maintenance_Due ORDER BY status DESC")
...
def analytics():
    ...
    # Query 47: Avg response time per area (nested subquery)
    area_resp = query(
        """SELECT location AS area,
                  COUNT(*) AS total,
                  ROUND(AVG(resp),1) AS avg_resp,
                  MAX(resp) AS max_resp
           FROM (
               SELECT er.location,
                      TIMESTAMPDIFF(MINUTE, dr.dispatch_time, dr.arrival_time) AS resp
               FROM Emergency_Request er
               JOIN Dispatch_Record dr ON er.request_id = dr.request_id
               WHERE dr.arrival_time IS NOT NULL
           ) AS sub
...
def api_area_response():
    ...
    data = query(
        """SELECT location AS area,
           ...
           GROUP BY location ORDER BY avg_resp DESC LIMIT 10"""
    )
    return jsonify(data)
